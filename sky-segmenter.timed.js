/**
 * sky-segmenter.timed.js — 計測用の一時的な複製
 *
 * sky-segmenter.js に工程ごとのタイマーを入れただけのもの。処理は一切変えていない。
 * ボトルネックが判明したら捨てて、本体に戻すこと。
 *
 * 使い方:
 *   import { createSkySegmenter } from './sky-segmenter.timed.js';
 *   // 数十フレーム回したあと
 *   console.table(segTimings());        // 直近60フレームの中央値(ms)
 *
 * 注意: 実機では必ず本番と同じ条件で測ること（カメラモード・同じ解像度・
 * 画面を消さない）。熱ダレで数字が変わるので、開始直後と1分後の両方を見る。
 */
/**
 * sky-segmenter.js
 *
...
export async function createSkySegmenter(options = {}) {
  ...
  async function segment(source) {
    ...
    const mk = (k) => { mark(k, performance.now() - t); t = performance.now(); };
    ...
    mk('7_ガイデッドフィルタ');
    mark('0_合計', performance.now() - whole);
...
// ---- 計測（この複製にだけ入っている） ----
const KEEP = 60;                       // 直近このフレーム数の中央値を出す
const RING = new Map();
...
/** 直近 60 フレームの工程別中央値(ms)。console.table(segTimings()) で見る。 */
export function segTimings() {
  const out = {};
  for (const [k, a] of [...RING].sort()) {
    const s = [...a].sort((x, y) => x - y);
    out[k.slice(2)] = { ms: +s[s.length >> 1].toFixed(2), n: a.length };
  }
  return out;
}
globalThis.segTimings = segTimings;
