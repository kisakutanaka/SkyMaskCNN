...
**合成は次のフレームを投げたあとに行います。** 素直に「待つ → 合成 → 次を投げる」と
書くと、メインスレッドの GPU 仕事（控えの `drawImage`・合成・`transferToImageBitmap`）と
Worker の推論が**直列に並びます**。実機ではそれで 1 枚 50ms のうち 24ms が Worker の
外に出ていました（Worker 内は 25ms）。先に次を投げてから前の対を合成すると、
その 2 つが重なります。
