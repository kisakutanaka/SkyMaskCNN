...
**実機 iPhone SE (第3世代) で 1 フレーム 20.9ms（開始直後）〜38.1ms（約1分後）。
同じ端末で SkyMaskCV は 7.5ms。** 30fps の予算 33ms に対して、**冷えているうちは
1.6 倍の余裕で収まり、熱ダレすると 15% ほど外れます**。

内訳を追った結果、**残っているコストはほぼ推論そのもの**です。取り込み・読み戻し・
合成といった周辺は、工夫でほぼ消えました（1 フレームのうち 1.5〜3.7ms）。

| | 開始直後 | 約1分後 |
|---|---|---|
| **1 フレーム** | **20.87 ms**（48fps相当） | **38.13 ms**（26fps相当） |
| うち分離（Worker） | 19.33 ms | 34.47 ms |
| うち控え・合成（メイン） | 1.53 ms | 3.67 ms |

読み戻し (`getImageData`) は当初 19ms で最大のコストでしたが、**1〜2ms まで
落ちました**（下記「速度」）。いま効いているのは**熱ダレで計算が一様に 2 倍近く
遅くなること**で、ここから先はモデルを替えるしかありません。
...
**実機 iPhone SE (第3世代) / Safari**（カメラ・refineSize 512・15枚ごとの平均）:

| 経過 | 1 フレーム | 分離 | 控え・合成 |
|---|---|---|---|
| 開始直後 | 20.87 ms (48fps相当) | 19.33 | 1.53 |
| 約1分後 | 38.13 ms (26fps相当) | 34.47 | 3.67 |

分離の中の内訳は、工程ごとにタイマーを入れた版で測ったものです（計測の足場は
役目を終えたので本体からは外してあります）。熱ダレ前後で一様に 2 倍近くなります。

| 工程 | 開始直後 | 1分後 |
|---|---|---|
| 推論 | 13 ms | 23 ms |
| ガイデッドフィルタ | 3 ms | 5 ms |
| `getImageData` | 1 ms | 2 ms |
| 縮小と正規化 | 1 ms | 2 ms |

手元の Mac + Chrome（ヘッドレス, `--disable-gpu`）では 16ms/frame（分離 11.6 /
控え・合成 4.8）。
...
- 熱ダレ後は 1 フレーム 38ms まで落ちる（開始直後は 21ms）
- 右ペインは分離 1 枚分だけ古いフレームを映す。左の元映像とは 21〜38ms ずれる
